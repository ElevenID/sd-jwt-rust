...
use sd_jwt_rs::issuer::issuance_benchmark::{
    issuance_benchmark_cases, IssuanceBenchmarkFixture, IssuanceBenchmarkRoute,
...
fn benchmark_issuance(c: &mut Criterion) {
    ...
    for case in issuance_benchmark_cases() {
        ...
        // route observed at that same stage's preflight boundary.
        for (stage, candidate_route) in [
            ...
            println!("{}", candidate_route.machine_record(case, stage));
        }
