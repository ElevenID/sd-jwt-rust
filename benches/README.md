...
medium nested objects, bounded large strings, and a mixed-size sequence. Large
values share a fixed raw-byte budget and are individually capped so every
fixture remains below the public encoded-disclosure and presentation limits. Criterion
...
The large and mixed cases target enough aggregate encoded disclosure data to
exercise the parallel byte gate at eligible disclosure counts while retaining
headroom for JSON, Base64url expansion, the issuer JWT, and compact separators.
...
Do not use the former fixed-64-KiB harness for cross-revision comparisons. Its
large fixtures exceed the verifier's public limits, and its IDs and mixed
payloads are not comparable with this bounded matrix. A historical comparison
is valid only when the same portable bounded harness is applied unchanged to
both revisions, every benchmark ID and fixture fingerprint matches, build and
Criterion directories are isolated, and both execution orders agree. No
portable cross-revision harness is currently qualified here.
